...
// functions from GameDev.net for swapping bytes
...
RicVolumeSet::RicVolumeSet() {
	...
	TE2 = 0;
	TR = 0;
	NIFTIorientation.emptyStatus = false;
...
RicVolumeSet::RicVolumeSet(string fname) {
	...
	else if (fname.find(".hdr") != string::npos
			|| fname.find(".HDR") != string::npos
			|| fname.find(".nii") != string::npos
			|| fname.find(".NII") != string::npos) {
	//	cout << "\ndebug: .hdr||.nii file";
...
int RicVolumeSet::Write(string fname) {
	...
	} else if (fname.find(".nii") != string::npos
			|| fname.find(".NII") != string::npos) {
...
int RicVolumeSet::Read_NEMA(string name) {
	...
	if (this->orientation.compare(3, 1, "-") != 0)
		xdir = -1;
	if (this->orientation.compare(4, 1, "-") != 0)
		ydir = -1;
	if (this->orientation.compare(5, 1, "-") != 0)
		zdir = -1;
	...
	for (int n = 0; n < nvol; ++n) {
		...
		} else if (ntype == NEMA_SIGNED) // signed short
		{
			...
			matrix(&ctmp, ny, nx);
			for (i = 0; i < nz; ++i) {
				nread = fread(&ctmp[0][0], sizeof(short), ntoread, fptr);
				if (ferror(fptr)) {
					cerr << "Error reading file" << nfile[0].fname(0) << endl;
					return 0;
				}
...
int RicVolumeSet::Write_NEMA(string fname, string orient_str) {
	...
	if (orient_str.length() != 6) // must be 6 characters
			{
		cerr << "Write_NEMA - invalid orientation string length" << orient_str
				<< endl;
		return 0;
	}
	string xyz = orient_str.substr(0, 3); // must be of type XYZ
	if (xyz.find("XYZ") == string::npos) {
		cerr << "Write_NEMA - invalid orientation string prefix" << orient_str
				<< endl;
		return 0;
	}
...
int RicVolumeSet::Read_NIFTI(string fname) {
	...
	if (nfiletype == NIFTI_FTYPE_ANALYZE)
		this->filetype = RIC_ANALYZE_FILE;
	else if (nfiletype == NIFTI_FTYPE_NIFTI1_1
			|| nfiletype == NIFTI_FTYPE_NIFTI1_2)
		this->filetype = RIC_NIFTI_FILE;
...
//	this->nbits =  nim->nbyper*8;
	...
	mat44 qto_xyz = nim->qto_xyz;
	//Quatern data uploaded Edit Brian Donohue
	this->NIFTIorientation.b = nim->quatern_b;
	this->NIFTIorientation.c = nim->quatern_c;
	this->NIFTIorientation.d = nim->quatern_d;
	this->NIFTIorientation.qx = nim->qoffset_x;
	this->NIFTIorientation.qy = nim->qoffset_y;
	this->NIFTIorientation.qz = nim->qoffset_z;
	this->NIFTIorientation.dx = this->dx;
	this->NIFTIorientation.dy = this->dy;
	this->NIFTIorientation.dz = this->dz;
	this->NIFTIorientation.qfac = nim->qfac;
	this->NIFTIorientation.emptyStatus = false;
        for(size_t i = 0; i < 4; i++){
          for(size_t r = 0; r < 4; r++){
                this->NIFTIorientation.qto_xyz.m[i][r] = nim->qto_xyz.m[i][r];
	this->NIFTIorientation.qto_ijk.m[i][r] = nim->qto_ijk.m[i][r];
	this->NIFTIorientation.sto_xyz.m[i][r] = nim->sto_xyz.m[i][r];
	this->NIFTIorientation.sto_ijk.m[i][r] = nim->sto_ijk.m[i][r];
		}
	}



	//void ** bricks = brick_list.bricks;

	// clean up memory
	//nifti_free_NBL(&brick_list);
	nifti_image_free(nim);

	// allocate new volumes
	this->Init(nnx, nny, nnz, nbricks);
	...
	//cerr<<"Orientation is disabled"<<endl;
	//cout << "\nOrientation";

	//for ( n=0 ; n<this->nvol ; ++n ){
	this->orientation = convertNiftiSFormToNEMA(qto_xyz);
	...
	for (n = 0; n < this->nvol; ++n) {
		...
		/*Edit Brian Donohue*/
		 ...
		 int xIdx = this->orientation.find("X",0);
		 int yIdx = this->orientation.find("Y",0);
		 int zIdx = this->orientation.find("Z",0);
		 ...
		 int xdir=1,ydir=1,zdir=1;
		 if ( this->orientation.compare(xIdx+3,1,"-") != 0 ) xdir = -1;
		 if ( this->orientation.compare(yIdx+3,1,"-") != 0 ) ydir = -1;
		 if ( this->orientation.compare(zIdx+3,1,"-") != 0 ) zdir = -1;
...
int RicVolumeSet::Write_NIFTI(string fname) {
	...
	nim->qform_code = 2;           // codes for (x,y,z) space meaning
	nim->sform_code = 2;           // codes for (x,y,z) space meaning
	...
	int Zsign = 1;
/*
	if(this->orientation[this->orientation.find('X',0) + 3] == '-'){
		Xsign = -1;
	}
	if(this->orientation[this->orientation.find('Y',0) + 3] == '-'){
		Ysign = -1;
	}
	if(this->orientation[this->orientation.find('Z',0) + 3] == '-'){
		Zsign = -1;
	}
	bool found = false;
	for(int i = 0 ; i <= 1; i++){
		...
		for(int j = 0;j <= 1; j++){
			...
			for(int k = 0;  k <= 1; k++){
				...
				if((Xcom == Xsign) &&(Ycom == Ysign) && (Zcom == Zsign) ){
					nim->quatern_b = i;
					nim->quatern_c = j;
					nim->quatern_d = k;
					found = true;
...
/*
	...
	mat33 Comp;
	Comp = nifti_mat33_mul(Ymatrix, Zmatrix);
	Comp = nifti_mat33_mul(Comp, Xmatrix);

	matrix.m[3][3] = 1.f;
	float qw = sqrt(1.f + Comp.m[0][0] + Comp.m[1][1] + Comp.m[2][2])/2.f;
	nim->quatern_b = (Comp.m[2][1] - Comp.m[1][2])/(4.f * qw);
	nim->quatern_c = (Comp.m[0][2] - Comp.m[2][0])/(4.f * qw);
	nim->quatern_d = (Comp.m[1][0] - Comp.m[0][1])/(4.f * qw);
...
*/
	...
	//Edit by: Brian Donohue



//	if(this->NIFTIorientation.emptyStatus == false){
		nim->quatern_b=this->NIFTIorientation.b;
	    nim->quatern_c=this->NIFTIorientation.c;
     	nim->quatern_d=this->NIFTIorientation.d;
    	nim->qoffset_x=this->NIFTIorientation.qx;
    	nim->qoffset_y= this->NIFTIorientation.qy;
    	nim->qoffset_z= this->NIFTIorientation.qz;
    	nim->dx = this->NIFTIorientation.dx;
    	nim->dy = this->NIFTIorientation.dy;
    	nim->dz = this->NIFTIorientation.dz;
	    nim->qfac = this->NIFTIorientation.qfac;
	    nim->qto_xyz = nifti_quatern_to_mat44( this->NIFTIorientation.b, this->NIFTIorientation.c, this->NIFTIorientation.d,
			this->NIFTIorientation.qx, this->NIFTIorientation.qy, this->NIFTIorientation.qz,
		this->NIFTIorientation.dx, this->NIFTIorientation.dy, this->NIFTIorientation.dz, this->NIFTIorientation.qfac);
	    nim->qto_ijk = nifti_mat44_inverse(nim->qto_xyz);
         nim->sto_ijk = this->NIFTIorientation.sto_ijk;
         nim->sto_xyz = this->NIFTIorientation.sto_xyz;
         nim->datatype = 16;
        nim->nbyper = 4;
...
//	}
	...
	if (nim->datatype == DT_FLOAT) {
		...
		for (n = 0; n < this->nvol; ++n) {
			...
			{
				for (i = 0; i < this->nz ; i++) {
					for (j = 0; j < this->ny; j++) // swap order here too
							{
						for (k = 0; k < this->nx; k++) {
			...
			{
				for (i = 0; i < this->nz ; i++) // swap slice order
						{
					for (j = 0; j < this->ny; j++) {
						for (k = 0; k < this->nx; k++) {
							*bptr = (float) (this->VolSet[n].vox[k][j][i]);
			...
			if (nim->nifti_type == NIFTI_FTYPE_ANALYZE) // swap slice order
			{
				for (i = 0; i < this->nz; i++) {
					for (j = 0; j < this->ny; j++) // swap order here too
						...
						for (k = 0; k < this->nx; k++) {
							*bptr =
									(short) (this->VolSet[n].vox[k][j][i] + 0.5);
							bptr++;
						}
					}
				}
			...
			if (nim->nifti_type == NIFTI_FTYPE_ANALYZE) // swap slice order
			{
				for (i = 0; i < this->nz; i++)  {
					for (j = 0; j < this->ny; j++){  // swap order here too
							...
							for (k = 0; k < this->nx; k++) {
							*bptr =
									(unsigned char) (this->VolSet[n].vox[k][j][i]
											+ 0.5);
							bptr++;
						}
					}
...
int RicVolumeSet::Read_GIS(string fname) {
	...
	if (fptr == NULL) {
		cerr << "RicVolumeSet::Read_GIS - cannot open file: " << dimname
				<< endl;
		return 0;
	}
	...
	while (fgets(line, 100, fptr) != NULL) {
		if (strncmp(line, "-bo", 3) == 0) // byte order
			...
			if (strncmp(bo, "ABCD", 4) == 0) // big endian
				...
				cerr << "RicVolumeSet::Read_GIS - unknown byte order " << bo
						<< endl;
				return 0;
			}
			...
			if (strncmp(type, "S16", 3) == 0 || strncmp(type, "s16", 3) == 0) // unsigned 16
				...
				cerr << "RicVolumeSet::Read_GIS - unknown data type " << type
						<< endl;
				return 0;
			}
			...
			if (strncmp(imode, "binar", 5) == 0) {
				...
				cerr << "RicVolumeSet::Read_GIS - unknown image mode " << imode
						<< endl;
				return 0;
			}
		}
	...
	if (fptr == NULL) {
		cerr << "RicVolumeSet::Read_GIS - cannot open file: " << dataname
				<< endl;
		return 0;
	}
			...
			for (i = 0; i < nz; ++i) {
				for (j = 0; j < ny; ++j) {
					...
					for (k = 0; k < nx; ++k) {
						...
						if (bswap)
							val = (rowarray[k] >> 8) | (rowarray[k] << 8);
						else
							val = (short) rowarray[k];
